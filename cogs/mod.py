    ...
    async def timeout(
        ...
        try:
            ...
            if duration.total_seconds() != 0:
                ...
                embed = discord.Embed(
                    description=(
                        f"<@{member.id}> в тайм-ауте на {days} дней {hours} часов "
                ...
                await ctx.respond(
                    "Вы не можете отправить пользователя в тайм-аут без определения срока!",
                    ephemeral=True,
                )
        except discord.errors.Forbidden:
            await ctx.respond(
                f'У вас нет прав на мьют пользователя. Для выдачи тайм-аута обратитесь к <@&{server_data.get("mod_role_id")}>.',
                ephemeral=True,
            )
