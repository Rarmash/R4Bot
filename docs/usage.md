...
Перед первым запуском нужен минимальный `servers.json`, иначе ядро не узнает, какие встроенные модули загрузить:
...
Команда добавит блок настроек для текущего сервера:
...
- загружает built-in модули из `servers.json`;
