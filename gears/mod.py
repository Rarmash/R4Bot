...
class Mod(commands.Cog):
    ...
    async def timeout(self, ctx, member: discord.Member, reason, days: discord.Option(int, max_value = 27, default = 0, required = False), hours: discord.Option(int, max_value = 23, default = 0, required = False), minutes: discord.Option(int, max_value = 59, default = 0, required = False), seconds: discord.Option(int, max_value = 59, default = 0, required = False)):
        server_data = self.servers_data.get(str(ctx.guild.id))
        if not server_data:
            return
